...
CONFIG = {
    ...
    "A_MAX": math.pi / 2,         # a_j = A_MAX * tanh(beta)
    "INIT_BETA":100,
    ...
    "LR": 0.1,
    "FD_EPS": 1e-20,
    "LOSS_TOL_LAST5": 1e-20,
    "PATIENCE_LAST5": 5,

    # poda / aceitação
    "COEF_ZERO_TOL": 1e-20,
    "GLOBAL_IMPROVEMENT_TOL": 1e-20,
    "SATURATION_PATIENCE": 10,
    "PROB_SUPPORT_TOL": 1e-14,

    # saída
    "OUTDIR": "results_maxcut_walsh_seq_v3",
    "SHOW_PLOTS": False,
}
...
def run_current_global_circuit(
    ...
    summary["full_state"] = out["full_state"]
    return summary
...
def train_one_unitary_local_then_eval_global(
    ...
    outdir: Path,
    cut_vals: np.ndarray,
    prev_global_expected_cut: float,
) -> Tuple[StepResult, Dict[int, float], Dict[str, object]]:
    ...
    final_coef_candidate = bounded_coef(best_beta, a_max)

    accepted_coeffs = dict(frozen_coeffs)
    kept = False
    global_metrics_after_step: Dict[str, object]

    if abs(final_coef_candidate) >= coef_zero_tol:
        tentative_coeffs = dict(frozen_coeffs)
        tentative_coeffs[j] = float(final_coef_candidate)
        tentative_global = run_current_global_circuit(
            n=n,
            coeffs=tentative_coeffs,
        ...
        improvement = float(tentative_global["expected_cut"]) - float(prev_global_expected_cut)
        if improvement >= float(cfg["GLOBAL_IMPROVEMENT_TOL"]):
            kept = True
            accepted_coeffs = tentative_coeffs
            global_metrics_after_step = tentative_global
        else:
            global_metrics_after_step = run_current_global_circuit(
                n=n,
                coeffs=frozen_coeffs,
                reverse_bits=reverse_bits,
                er=er,
                cut_vals=cut_vals,
                support_tol=support_tol,
            )
            stop_reason += "_rejected_by_global_loss"
    else:
        global_metrics_after_step = run_current_global_circuit(
            n=n,
            coeffs=frozen_coeffs,
    ...
    step = StepResult(
        ...
        coefficient=float(final_coef_candidate if kept else 0.0),
        local_loss=float(best_loss),
        local_expected_cut=float(best_metrics["expected_cut"]),
        local_best_cut=float(best_metrics["most_probable_cut"]),
    ...
    return step, accepted_coeffs, global_metrics_after_step
...
def save_global_plots(steps: List[StepResult], outdir: Path, show_plots: bool = False) -> None:
    ...
    plt.bar(js, np.abs(coeffs))
    plt.xlabel("j")
    plt.ylabel("|coeficiente|")
    plt.title("Magnitude dos coeficientes Walsh finais")
    plt.tight_layout()
    plt.savefig(plot_dir / "trained_coefficients_magnitude.png", dpi=180)
    if show_plots:
        plt.show()
    plt.close()
...
def run_sequential_walsh_maxcut(cfg: Dict[str, object]) -> Dict[str, object]:
    ...
    initial_global = run_current_global_circuit(
        n=n,
        coeffs=coeffs,
        reverse_bits=reverse_bits,
        er=float(cfg["ER"]),
    ...
    prev_global_expected_cut = float(initial_global["expected_cut"])

    max_terms = cfg["MAX_TERMS"]
    if max_terms is None:
        candidate_js = list(range(0, 2**n))
    else:
        candidate_js = list(range(0, min(2**n, int(max_terms))))

    no_improve_counter = 0

    for j in candidate_js:
        step, coeffs, global_metrics = train_one_unitary_local_then_eval_global(
            j=j,
            ...
            outdir=outdir,
            cut_vals=cut_vals,
            prev_global_expected_cut=prev_global_expected_cut,
        )
        steps.append(step)

        improvement = float(global_metrics["expected_cut"]) - float(prev_global_expected_cut)
        if improvement < float(cfg["GLOBAL_IMPROVEMENT_TOL"]):
            no_improve_counter += 1
        else:
            no_improve_counter = 0
        prev_global_expected_cut = float(global_metrics["expected_cut"])

        print(
            f"j={j:3d} | kept={step.kept} | coef={step.coefficient:+.6f} | "
            f"local_loss={step.local_loss:.6f} | global_E[cut]={step.global_expected_cut_after_step:.6f} | "
            f"global_most_prob_cut={step.global_most_probable_cut_after_step:.6f}"
        )

        if no_improve_counter >= int(cfg["SATURATION_PATIENCE"]):
            print("Saturou globalmente: parando adição de novos termos.")
            break
    ...
    final_js = [idx for idx, val in sorted(coeffs.items()) if abs(val) > 0.0]
    ...
    summary = {
        ...
        "active_indices": [int(k) for k, v in sorted(coeffs.items()) if abs(v) > 0.0],
